...
def print_grid():
    ...
    for count in range(1,5):
        print('  +-----+-----+-----+-----+') 
        print('{} |{:^5}|{:^5}|{:^5}|{:^5}|'.format(count,grid[row][0],grid[row][1],grid[row][2],grid[row][3]))
        row = row + 1
    print('  +-----+-----+-----+-----+') # print bottom row
    ...
    if result == True:
        ...
        # turn 2 onwards must build orthogonally adjacent to a building
            ...
            if up_row >= 0 and grid[up_row][up_col] != '     ':
                grid[row_number][col_number] = building_choice
            elif down_row <= 3 and grid[down_row][down_col] != '     ':
                grid[row_number][col_number] = building_choice
            elif left_col >= 0 and grid[left_row][left_col] != '     ':
                grid[row_number][col_number] = building_choice
            elif right_col <= 3 and grid[right_row][right_col] != '     ':
                grid[row_number][col_number] = building_choice
            else:
...
def display_scores(building_scores,cell):
    ...
    if len(building_scores) > 0:
        ...
        print('{}: '.format(cell) + str(building_scores[0]) + output + ' = ' + str(sum(building_scores)))
...
def current_score():
    ...
    # total score
    total_score = sum(hse_scores + fac_scores + shp_scores + hwy_scores + bch_scores)
